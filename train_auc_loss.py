...
def train():
    ...
    model = Xception(cfg.input_shape, include_top=True, classes=len(xray14_labels), pretrain_weights='imagenet')
    model.compile(optimizer=Adam(), loss=[roc_auc_score], metrics=['binary_accuracy', 'mae'])
    log_path = os.path.join(cfg.log_dir, 'xray14_auc')
    makedir(log_path)
